...
const StockAnalyzer = ({ isOpen, onClose, availableCoins, account, provider }) => {
  ...
  const executeTrade = async () => {
    ...
    try {
      ...
      if (tradeType === 'buy') {
        const ethPriceUsd = selectedCoin.current_price;
        const amountUsd = tradeAmount * ethPriceUsd;
        ...
        alert(`Sell executed: Funds for ${tradeAmount} ETH worth of ${selectedCoin.symbol.toUpperCase()} will be sent to ${linkedUpiId}`);
      }
    } catch (err) {
